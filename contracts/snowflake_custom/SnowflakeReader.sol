 ...
 * @dev Intended to provide a simple thing for contracts to extend and do things such as read EINs. For now, this
 * is its only function, but this is needed for design purposes, IMO
 */
contract SnowflakeReader {
    ...
    function getEIN(address einAddress) internal returns (uint256 ein) {
        ...
        address identityRegistryAddress = snowflake.identityRegistryAddress();

        IdentityRegistryInterface identityRegistry = IdentityRegistryInterface(identityRegistryAddress);
